...
def apply_patches(patch_path_iter, tree_path, reverse=False, patch_bin_path=None):
    ...
    logger = get_logger()
    patch_num = 0
    while patch_num < len(patch_paths):
        patch_path = patch_paths[patch_num]
        current_patch_num = patch_num + 1
        ...
        logger.info('* %s %s (%s/%s)', log_word, patch_path.name, current_patch_num, len(patch_paths))
        logger.debug(' '.join(cmd))
        
        try:
            subprocess.run(cmd, check=True)
            # Success, move to next patch
            patch_num += 1
        except subprocess.CalledProcessError as e:
            logger.error('Failed to apply patch %s', patch_path.name)
            ...
            # Try to identify the file that failed
            stderr_str = e.stderr if isinstance(e.stderr, str) else (e.stderr.decode('utf-8', errors='replace') if e.stderr else "")
            failed_files = []
            for line in stderr_str.splitlines():
                if "patch failed:" in line or "patch does not apply" in line:
                    failed_file = line.split(":", 1)[0].strip().replace("patch failed: ", "")
